...
use reqwest::{Result as ApiResult, Url};
...
trait WeatherFuture = Future<Item = reqwest::async::Response, Error = reqwest::Error>;
...
impl<'a> ApiClient<'a> {
    ...
    pub fn get_forecast<'b, T>(&self, request: T) -> impl WeatherFuture
    where
        T: Borrow<ForecastRequest<'b>> + Sized,
    {
        self.client
            .get(request.borrow().url.clone())
    ...
    /// `reqwest::Client.get(..)`, so it will return an error under the
    /// same conditions in which reqwest would.
    pub fn get_time_machine<'b, T>(&self, request: T) -> impl WeatherFuture
    where
...
mod tests {
    use super::{
        ExcludeBlock, ExtendBy, ForecastRequest, ForecastRequestBuilder, Lang, TimeMachineRequest,
        TimeMachineRequestBuilder, Units, EXCLUDE, EXTEND, FORECAST_URL, LANG, UNITS,
    };
    ...
    fn test_norwegian_lang_serde() {
        ...
        let test_struct_deserialized: TestStruct =
            serde_json::from_str(test_struct_serialized.as_str()).unwrap();
    ...
    fn test_integration() {
        ...
        // let time_machine_response = api_client
